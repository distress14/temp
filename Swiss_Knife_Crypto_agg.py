...
#XOR Function Between 2 string - They must be of the same length
def xor(a, b):
    if len(a) != len(b):
        raise Exception('Error in rule 1')
...
#eliminare gli spazi della stringa
input = input.strip()
...
XOR_KEY='A' # can be only letters

# read the file with the encrypted message
with open('encrypted', 'r') as f:
    encrypted_message=f.read()

string2binary(encrypted_message)
print(encrypted_message)
'''
for _ in range(len(encrypted_message)):
    res = xor(encrypted_message, XOR_KEY)
    XOR_KEY = 
'''
...
for i in range(26):
    for j in range(len(encrypted_message)):
        enc= encrypted_message.split[:8]

'''
'''
for c in range(24):
    cryptedMessage = ''.join(chr(ord(c)^ord(k)) for c,k in zip(encrypted_message, cycle(XOR_KEY)))
    print(cryptedMessage)
    print()
    XOR_KEY = chr(ord(XOR_KEY)+c)

XOR_KEY = "a"
print("Lettere Minuscoleeeeee SEsso Gay per tutti gli abbonati al servizio LGBTQWSP+ [Bear]")

for z in range(24):
    cryptedMessage = ''.join(chr(ord(z)^ord(k)) for z,k in zip(encrypted_message, cycle(XOR_KEY)))
    print(cryptedMessage)
    print()
    XOR_KEY = chr(ord(XOR_KEY)+z)
'''
